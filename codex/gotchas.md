...
25. **World swapping must replace only the active SplatMesh, not the renderer/camera/listeners.** Re-initializing viewer on each world load can duplicate input listeners and leak render loops. Keep a single viewer instance and hot-swap just the world mesh (`loadWorld` + `disposeCurrentWorld`), then rebuild spatial index/manifest for that mesh.
